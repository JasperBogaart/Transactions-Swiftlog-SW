...
def create_report(source_path, output_path):
    ...
    activities = list(MINUTES_PER_ACTIVITY)
    users = sorted(
    ...
    productivity_ws = create_productivity_sheet(wb, users, activities, counts)
    copy_column_widths(productivity_ws, ws, last_col)
...
def read_report_counts(report_path):
...
def create_merged_report(report_paths, output_path):
    ...
    for report_path in report_paths:
        report_users, report_counts = read_report_counts(report_path)
    ...
    productivity_ws = create_productivity_sheet(wb, users, activities, combined_counts)
...
def create_productivity_sheet(wb, users, activities, counts):
    ...
    for activity in activities:
        ...
        minute_total = count_total * MINUTES_PER_ACTIVITY.get(activity, 0)
        activity_minutes.append(minute_total)
    ws.append(["TOTAAL"] + activity_minutes + [sum(activity_minutes) / 60])
    ...
    for user in users:
        ...
        for activity in activities:
            ...
            minutes = activity_count * MINUTES_PER_ACTIVITY.get(activity, 0)
